...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    if(nBestHeight > 2365200)
    {
        obj.push_back(Pair("stakeinterest",    (uint64_t)COIN_YEAR_REWARD5));
    }
    if(nBestHeight > 788400)
    {
        obj.push_back(Pair("stakeinterest",    (uint64_t)COIN_YEAR_REWARD4));
    }
    if(nBestHeight > 262800)
    {
        obj.push_back(Pair("stakeinterest",    (uint64_t)COIN_YEAR_REWARD3));
    }
    if(nBestHeight > 131400)
    {
        obj.push_back(Pair("stakeinterest",    (uint64_t)COIN_YEAR_REWARD2));
    }
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Cakeslices is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Cakeslices is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Cakeslices is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Cakeslices is downloading blocks...");
