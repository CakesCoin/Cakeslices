...
#if defined(NDEBUG)
# error "Cakeslices cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Cakeslices Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        // keys. (remCakeslice the 520 byte limit on redeemScript size) That works
...
int64_t GetProofOfWorkReward(int64_t nHeight, int64_t nFees)
{
    ...
    if(nHeight == 5)
    {
        nSubsidy = 222500000 * COIN; // 5% Premine for coinswap from CakeCoin
    }
    else if(nHeight > 5)
    {
        nSubsidy = 511 * COIN; // Standard PoW Subsidy
    }
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD1 * 33 / (365 * 33 + 8); // 10% (6-months)
    if(nBestHeight > 2365200) // Forever
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD5 * 33 / (365 * 33 + 8); // 1%
    }
    if(nBestHeight > 788400) // ~6 Years
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD4 * 33 / (365 * 33 + 8); // 5%
    }
    if(nBestHeight > 262800) // ~2 Years
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD3 * 33 / (365 * 33 + 8); // 250%
    }
    if(nBestHeight > 131400) // ~6 Months
    {
        nSubsidy = nCoinAge * COIN_YEAR_REWARD2 * 33 / (365 * 33 + 8); // 25%
    }
...
{
    RenameThread("Cakeslices-loadblk");
